...
const EMPTY: Omit<Project, 'id' | 'views' | 'created_at' | 'updated_at'> = {
  slug: '', title: '', excerpt: '', content: '', cover_image: '',
  tags: '', github_url: '', demo_url: '', architecture: '',
...
const STYLES = `
  ...
  .ProseMirror {
    padding: 1.1rem 1.25rem; min-height: 320px; outline: none;
    color: var(--adm-text); font-size: 16px; line-height: 1.8;
    font-family: system-ui, sans-serif;
  }
  .ProseMirror h1 { font-size: 1.85em; font-weight: 700; margin: 1.4em 0 0.5em; }
  .ProseMirror h2 { font-size: 1.4em;  font-weight: 600; margin: 1.3em 0 0.45em; }
  .ProseMirror h3 { font-size: 1.15em; font-weight: 600; margin: 1.1em 0 0.4em; }
  ...
  .ProseMirror pre { background: var(--adm-border); padding: 1rem 1.25rem; border-radius: 8px; overflow-x: auto; margin: 1.1em 0; }
  ...
  @media (max-width: 600px) {
    .proj-card { flex-wrap: wrap !important; }
    .proj-card-actions { width: 100% !important; border-top: 1px solid var(--adm-border); padding-top: 10px; margin-top: 2px; }
...
function Toolbar({ editor }: { editor: ReturnType<typeof useEditor> }) {
  ...
  return (
    <div className="rich-toolbar">
      <TBtn active={false} onClick={() => editor.chain().focus().undo().run()} title="Deshacer">↩</TBtn>
      <TBtn active={false} onClick={() => editor.chain().focus().redo().run()} title="Rehacer">↪</TBtn>
      <div className="rich-toolbar-sep" />
      <TBtn active={editor.isActive('bold')} onClick={() => editor.chain().focus().toggleBold().run()} title="Negrita"><strong>B</strong></TBtn>
      <TBtn active={editor.isActive('italic')} onClick={() => editor.chain().focus().toggleItalic().run()} title="Cursiva"><em>I</em></TBtn>
      <TBtn active={editor.isActive('underline')} onClick={() => editor.chain().focus().toggleUnderline().run()} title="Subrayado"><u>U</u></TBtn>
      <div className="rich-toolbar-sep" />
      <TBtn active={editor.isActive('heading', { level: 1 })} onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} title="H1">H1</TBtn>
      <TBtn active={editor.isActive('heading', { level: 2 })} onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} title="H2">H2</TBtn>
      <TBtn active={editor.isActive('heading', { level: 3 })} onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} title="H3">H3</TBtn>
      <div className="rich-toolbar-sep" />
      <TBtn active={editor.isActive('bulletList')} onClick={() => editor.chain().focus().toggleBulletList().run()} title="Lista">• Lista</TBtn>
      ...
      <TBtn active={false} onClick={() => editor.chain().focus().setHorizontalRule().run()} title="Separador">—</TBtn>
      <TBtn active={editor.isActive('link')} onClick={addLink} title="Enlace">↗ Link</TBtn>
...
function RichEditor({ initialContent, onChange }: { initialContent: string; onChange: (html: string) => void }) {
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({ placeholder: 'Describe el proyecto: objetivos, arquitectura, decisiones técnicas...' }),
    ],
...
export default function AdminProyectosPage() {
  ...
  const [form, setForm]           = useState<Omit<Project, 'id' | 'views' | 'created_at' | 'updated_at'>>(EMPTY);
  const [editId, setEditId]       = useState<number | null>(null);

  function loadProjects() {
    setLoading(true);
    fetch('/api/projects/list?admin=true')
      .then(r => r.json())
  ...
  function openEdit(p: Project) {
    ...
    setForm({
      ...
      cover_image: p.cover_image || '', tags: p.tags, github_url: p.github_url || '',
      demo_url: p.demo_url || '', architecture: p.architecture || '',
  ...
  async function handleSave() {
    if (!form.title || !form.slug) {
      setMsg('Título y slug son obligatorios.'); return;
    }
    ...
    try {
      ...
      if (data.ok) { setMsg('✓ Guardado'); loadProjects(); setView('list'); }
  ...
  async function handleDelete(id: number) {
    ...
    setDeleting(id);
    try {
      const res = await fetch('/api/projects/delete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
        ...
        {view === 'list' ? (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <div>
                <h1 style={{ color: 'var(--adm-text)', fontSize: '18px', fontWeight: 500 }}>Proyectos</h1>
                <p style={{ color: 'var(--adm-muted)', fontSize: '11px', marginTop: '2px' }}>{projects.length} proyectos</p>
              </div>
              <button className="abtn abtn-p" onClick={openNew}>+ Nuevo proyecto</button>
            </div>
            ...
            {!loading && projects.length === 0 && (
              <div style={{ background: 'var(--adm-card)', border: '1px solid var(--adm-border)', borderRadius: '10px', padding: '2rem', textAlign: 'center' }}>
                <p style={{ color: 'var(--adm-muted)', fontSize: '13px', marginBottom: '1rem' }}>No hay proyectos todavía.</p>
                <button className="abtn abtn-p" onClick={openNew}>Crear el primer proyecto</button>
              </div>
            )}

            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {projects.map(p => (
                <div key={p.id} className="proj-card" style={{ background: 'var(--adm-card)', border: '1px solid var(--adm-border)', borderRadius: '10px', padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  {p.cover_image && (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: 'var(--adm-muted)', fontSize: '11px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                      ...
                      {p.architecture && <><span>·</span><span>{p.architecture}</span></>}
                      <span>·</span>
                      <span>{p.views} visitas</span>
                      <span style={{ color: 'var(--adm-label)', fontFamily: 'monospace' }}>/projects?slug={p.slug}</span>
                    </div>
                  </div>
                  <div className="proj-card-actions" style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
          ...
          <>
            <div className="proj-editor-hdr" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              ...
              <div className="proj-editor-hdr-right" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  ...
                  <input style={{ ...inp, fontFamily: 'monospace', fontSize: '12px' }} value={form.slug} onChange={e => set('slug', slugify(e.target.value))} placeholder="unyona" />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  ...
                  <input style={inp} value={form.tags} onChange={e => set('tags', e.target.value)} placeholder="React, TypeScript, Tailwind, Express" />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  <label style={lbl}>URL Demo / Landing</label>
                  <input style={inp} value={form.demo_url} onChange={e => set('demo_url', e.target.value)} placeholder="https://unyona.com" />
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                  <label htmlFor="cover-proj-img" className="abtn abtn-g" style={{ display: 'inline-block', cursor: imgUploading ? 'not-allowed' : 'pointer', opacity: imgUploading ? 0.5 : 1 }}>
                  ...
                  <input id="cover-proj-img" type="file" accept="image/*" onChange={handleImageFile} disabled={imgUploading} style={{ display: 'none' }} />
                </div>
                {form.cover_image && (
              ...
              <div>
                <label style={lbl}>Case Study (contenido detallado)</label>
                <RichEditor
                  key={editorKey}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '20px', padding: '12px', background: 'var(--adm-card)', border: '1px solid var(--adm-border)', borderRadius: '8px' }}>
                <label style={{ color: 'var(--adm-text)', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  Publicado (visible en /projects)
                </label>
                <label style={{ color: 'var(--adm-text)', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <input type="checkbox" checked={form.featured === 1} onChange={e => set('featured', e.target.checked ? 1 : 0)} style={{ width: '16px', height: '16px', accentColor: 'var(--primary)' }} />
                  <span>⭐ Destacado</span>
                  <span style={{ fontSize: '10px', color: 'var(--adm-muted)' }}>(aparece primero en la lista)</span>
                </label>
              </div>
            </div>
          </>
