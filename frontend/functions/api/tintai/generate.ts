...
async function callClaude(apiKey: string, userPrompt: string): Promise<string> {
  // Promise.race para timeout — más compatible con Cloudflare Workers que AbortController
  const timeout = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error('Timeout: Claude tardó demasiado. Inténtalo de nuevo.')), 25000)
  );
