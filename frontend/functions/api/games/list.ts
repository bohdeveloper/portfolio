...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  try {
    ...
    ).all<{ id: number; name: string; slug: string; description: string; url: string; screenshot: string; is_top: number; vote_count: number; ai_generated: number; sort_order: number; created_at: string }>();
