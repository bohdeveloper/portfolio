...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    if (delta === 1) {
      // Cap en MAX_COUNT para evitar contadores desbordados
      await env.DB.prepare(
        `INSERT INTO blog_reactions (post_id, emoji, count) VALUES (?, ?, 1)
         ON CONFLICT(post_id, emoji) DO UPDATE SET count = MIN(${MAX_COUNT}, count + 1)`
